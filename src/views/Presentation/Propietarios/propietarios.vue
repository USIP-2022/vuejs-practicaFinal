...
defineProps({
  headers: {
    type: Array,
    default: () => ["#", "Nombres", "Apellidos", "Direccion", "Accion"],
  },
  rows: {
    type: Array,
    required: true,
...
export default {
  ...
  computed: {
    propietarios() {
      return this.$store.getters.getAllPropietariosget ;
    },
  },
  methods: {
    changeItem(id) {
      this.$router.push({
        name:'editPropietario',
        params:{
          id
        }
      });
    ...
    deleteItem(id) {
      ...
      this.$store.dispatch('deletePropietario', id)
      this.$store.getters['getDeleteItem', id]
    },
  },
};
</script>
